...
export default function DashboardScreen() {
    ...
    useEffect(() => {
        ...
        // 🛰️ RADAR USERS VERİ ÇEKME MOTORU
        ...
        <div style={{ display: 'flex', height: '100vh', background: '#000', color: '#fff', fontFamily: 'sans-serif', flexDirection: isMobile ? 'column' : 'row' }}>
            ...
            {isMobile && (
                <div style={{ padding: '15px 20px', background: '#080808', borderBottom: '1px solid #111', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <b style={{color:'#00B4FF', letterSpacing:'1px'}}>NEURAX RADAR</b>
                    <div style={{width:24}}/>
                </div>
            )}
            ...
            <div style={{ flex: 1, padding: isMobile ? '20px' : '40px', overflowY: 'auto' }}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:30}}>
                    ...
                    {!isMobile && <div style={{background:'#00B4FF22', color:'#00B4FF', padding:'5px 15px', borderRadius:20, fontSize:12, fontWeight:'bold'}}>{users.length} AJAN</div>}
                </div>
                                ...
                                return (
                                    <tr key={u.uid} style={{ borderBottom: '1px solid #0a0a0a', background: u.isPremium ? 'rgba(0,180,255,0.01)' : 'transparent' }}>
                                        <td style={{ padding: 20 }}>
                                            ...
                                            <br/><small style={{ color: '#333' }}>{u.email || u.uid.substring(0,10)}</small>
                                        </td>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: 20, backdropFilter:'blur(10px)' }}>
                    <div style={{ background: '#0A0A0A', padding: 40, borderRadius: 50, width: '100%', maxWidth: '420px', border: '1px solid #222', boxShadow:'0 20px 60px rgba(0,0,0,0.5)' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', padding: 20, background: '#111', borderRadius: 20, alignItems: 'center', border: permTarget.isPremium ? '1px solid #FFD70044' : '1px solid #222' }}>
                            ...
                            <input 
                                type="checkbox" 
                                style={{accentColor:'#00B4FF', width:25, height:25, cursor:'pointer'}} 
                                checked={permTarget.isPremium || false} 
                                onChange={e => update(ref(db, `Users/${permTarget.uid}`), { isPremium: e.target.checked })} 
                            />
                        </div>

                        <div style={{marginTop:25, padding:20, background:'#050505', borderRadius:20, border:'1px solid #111', textAlign:'center'}}>
                            <p style={{fontSize:12, color:'gray', marginBottom:10}}>Lisans Bitiş: {permTarget.expiryDate ? new Date(permTarget.expiryDate).toLocaleDateString() : 'BELİRLENMEDİ'}</p>
                            <button onClick={() => extendLicense(permTarget.uid)} style={{ width: '100%', padding: 18, background: '#00B4FF', color: '#000', fontWeight: '900', borderRadius: 15, border: 'none', cursor: 'pointer', fontSize:14 }}>
                                390 GÜNLÜK MÜHÜRÜ BAS
                            </button>
                        </div>
